...
# Example my @subnets = ("127.0.0.1/8","10.0.5.99","192.168.48.0/24","172.16.0.0/12","::1/128","2001:db8:b00b:/64","2001:db8::/32");
my @subnets;
my $subnet;
...
sub event_imaploginall($);
sub event_imaploginfailed($);
sub event_pop3login($);
...
sub init_rrd($)
{
	...
	if(! -f $rrd and ! $opt{'no-mail-rrd'}) {
		RRDs::create($rrd, '--start', $m, '--step', $rrdstep,
				...
				'DS:delivered:ABSOLUTE:'.($rrdstep*2).':0:U',
				'DS:ipv4:ABSOLUTE:'.($rrdstep*2).':0:U',
				'DS:ipv6:ABSOLUTE:'.($rrdstep*2).':0:U',
	...
	if(! -f $rrd_virus and ! $opt{'no-virus-rrd'}) {
        ...
        # queue rrd
        if(! -f $rrd_queue and ! $opt{'no-queue-rrd'}) {
        ...
        elsif(-f $rrd_queue and ! defined $rrd_queue) {
                $this_minute = RRDs::last($rrd_queue) + $rrdstep;
        }

        # imap rrd
       	if(! -f $rrd_imap and ! $opt{'no-imap-rrd'}) {
               	RRDs::create($rrd_imap, '--start', $m, '--step', $rrdstep,
   	                        'DS:imaplogin:ABSOLUTE:'.($rrdstep*2).':0:U',
                               	'DS:imaplogintls:ABSOLUTE:'.($rrdstep*2).':0:U',
                               	'DS:imaploginall:ABSOLUTE:'.($rrdstep*2).':0:U',
                               	'DS:imaploginfailed:ABSOLUTE:'.($rrdstep*2).':0:U',
                               	'DS:pop3login:ABSOLUTE:'.($rrdstep*2).':0:U',
       	...
       	elsif(-f $rrd_imap and ! defined $rrd_imap) {
               	$this_minute = RRDs::last($rrd) + $rrdstep;
       	}
	...
	if($prog =~ /^postfix\/(.*)/) {
		...
		if($prog eq 'smtp') {
			if($text =~ /\bstatus=sent\b/) {
				...
				if($text =~ /relay=\S+\[(\S+)\]\:\d+\,\s/) {
					my $relay = $1;
					my $ip = NetAddr::IP->new($relay);
...
#		}
			...
			event($time, 'virus');
                ...
                if($text =~ /imap-login\:/) {
					...
					if($text =~ /rip\=(\S+)\,\slip\=/) {
                        ...
                        elsif($text =~ /\(auth\sfailed\,\s(\d+)\sattempts\sin\s\d+\s\w+\)/) {
                                for (my $i = $1; $i > 0; $i--) {
                                        event($time, 'imaploginfailed');
                                }
                        }
                }
					...
					if($text =~ /rip\=(\S+)\,\slip\=/) {
						...
						if(is_ipv4($client)) {
							...
							event($time, 'ipv6login');
                                ...
                                for (my $i = $1; $i > 0; $i--) {
                                        event($time, 'pop3loginfailed');
                                }
                        }

                }
        }
        elsif ($prog eq 'courierpop3login') {
                if($text =~ /LOGIN,/) {
                        event($time, 'pop3login');
                        event($time, 'pop3loginall');
                }
	...
	elsif ($prog eq 'mailqueue') {
		if($text =~ /^active\:\s(\d+)\,\sdeferred\:\s(\d+)/) {
			if($1 > 0) {
				for (my $i = $1; $i > 0; $i--) {
					event($time, 'active');
				}
...
This script does parse syslog and updates the RRD databases in
